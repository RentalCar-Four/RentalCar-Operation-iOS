...
@interface RegularView () <UITextFieldDelegate> {
...
- (void)setLeftImage:(UIImage *)leftImage {
    ...
    UIImageView *leftPhoneImgView = [[UIImageView alloc] initWithFrame:CGRectMake(0, (self.frame.size.height - 24)*0.5, 48, 24)];
    leftPhoneImgView.image = leftImage;
    leftPhoneImgView.contentMode = UIViewContentModeScaleAspectFit;
    self.leftView = leftPhoneImgView;
...
- (void)reformatAsCardNumber:(UITextField *)textField
{
    ...
    NSString *cardNumberWithoutSpaces = [self removeNonDigits:textField.text
                                    andPreserveCursorPosition:&targetCursorPosition];//删除(通过遍历删除空格)或增加(对数字进行限制)
    ...
    NSString *cardNumberWithSpaces = [self insertSpacesEveryFourDigitsIntoString:cardNumberWithoutSpaces
                                                       andPreserveCursorPosition:&targetCursorPosition]; //添加时
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)rang replacementString:(NSString *)string {
