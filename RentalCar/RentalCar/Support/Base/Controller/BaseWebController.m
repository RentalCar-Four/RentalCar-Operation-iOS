...
@interface BaseWebController ()<WKUIDelegate,WKNavigationDelegate,WKDelegate,UITextFieldDelegate>
...
- (void)viewDidLoad {
    ...
    if ([_webTitle isEqualToString:@"问题详情"]) {
        [self addItemForLeft:RightBtn Title:@"人工认证" Titlecolor:[UIColor colorWithHexString:@"#25D880"] action:@selector(ProIdentityID) spaceWidth:-10];
    }
...
- (void)configUI {
    ...
    _webView = [[WKWebView alloc] initWithFrame:CGRectMake(0, NVBarAndStatusBarHeight, kScreenWidth, kScreenHeight-NVBarAndStatusBarHeight) configuration:configuration];
    //注册方法
    ...
    [_webView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:[APPUtil urlEncoding:_homeUrl]] cachePolicy:NSURLRequestReloadIgnoringLocalCacheData timeoutInterval:30]];
...
-(void)ProIdentityID{
    if ([self.delegate respondsToSelector:@selector(ProIdentityIDClick)]) {
        [self.delegate ProIdentityIDClick];
    }
}
@end
