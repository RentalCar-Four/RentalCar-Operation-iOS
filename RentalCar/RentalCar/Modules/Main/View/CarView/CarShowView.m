...
@interface CarShowView ()<UIScrollViewDelegate>
{
    ...
    StationCarItem *_currentCarItem;
}
@end
...
- (void)setCarItems:(NSMutableArray *)carItems {
    ...
    for (int i = 0; i< _carItems.count; i++) {
        ...
        StationCarItem *item = _carItems[i];
        ...
        if (i==0) {
            [self showCarStatusWithItem:item];
        }
    }
}

- (void)showCarStatusWithItem:(StationCarItem *)item{
    if (![APPUtil isBlankString:item.carStatus]) {
        switch ([item.carStatus integerValue]) {
            case 0: //空闲
            {
                //                    _useCarBtn.backgroundColor = kCarColor_Kongxian;
                _useCarBtn.userInteractionEnabled = YES;
                [_useCarBtn setTitle:@"预约用车" forState:0];
            }
                break;
            case 1: //租赁
            {
                //                    _useCarBtn.backgroundColor = kCarColor_Zulin;
                _useCarBtn.userInteractionEnabled = NO;
                [_useCarBtn setTitle:[NSString stringWithFormat:@"%@ 正在租赁中",item.operationName] forState:0];
            }
                break;
            case 2: //预约
            {
                //                    _useCarBtn.backgroundColor = kCarColor_Yuyue;
                _useCarBtn.userInteractionEnabled = NO;
                [_useCarBtn setTitle:[NSString stringWithFormat:@"%@ 正在预约中",item.operationName] forState:0];
            }
                break;
            case 3: //故障
            {
                //                    _useCarBtn.backgroundColor = kCarColor_Guzhang;
                _useCarBtn.userInteractionEnabled = NO;
                [_useCarBtn setTitle:@"车辆故障，请救援" forState:0];
            }
                break;
            case 4: //亏电
            {
                //                    _useCarBtn.backgroundColor = kCarColor_Kuidian;
                _useCarBtn.userInteractionEnabled = YES;
                [_useCarBtn setTitle:@"预约用车" forState:0];
            }
                break;
            case 5: //运维
            {
                //                    _useCarBtn.backgroundColor = kCarColor_Yunwei;
                NSString *memberID = [UserInfo share].memberId;
                if ([memberID isEqualToString:item.operationMemberId]) { //是自己运维的
                    _useCarBtn.userInteractionEnabled = YES;
                    [_useCarBtn setTitle:[NSString stringWithFormat:@"点击查看运维行程"] forState:0];
                } else {
                    _useCarBtn.userInteractionEnabled = NO;
                    [_useCarBtn setTitle:[NSString stringWithFormat:@"%@ 正在运维中",item.operationName] forState:0];
                }
            }
                break;
            case 6: //离线
            {
                //                    _useCarBtn.backgroundColor = kCarColor_Lixian;
                _useCarBtn.userInteractionEnabled = NO;
                [_useCarBtn setTitle:@"车辆离线，请救援" forState:0];
            }
                break;
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    _currentCarItem = self.carItems[_pageControl.currentPage];
    [self showCarStatusWithItem:_currentCarItem];
