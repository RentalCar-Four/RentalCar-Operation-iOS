...
+ (NSArray *) clusterAnnotationsForMapView:(MAMapView *)mapView forAnnotations:(NSArray *)pins blocks:(NSUInteger)blocks minClusterLevel:(NSUInteger)minClusterLevel
{
    ...
    MAMapRect mapRect = MAMapRectWorld;
    ...
    MAMapRect visibleMapRect = MAMapRectMake(tileStartX, tileStartY, tileWidth*(BLOCKS+1), tileHeight*(BLOCKS+1));
    
    for (id<MAAnnotation> point in pins)
    {
        MAMapPoint mapPoint = MAMapPointForCoordinate(point.coordinate);
        if( MAMapRectContainsPoint(visibleMapRect,mapPoint) )
        {
    ...
    {
        MAMapPoint mapPoint = MAMapPointForCoordinate(pin.coordinate);
...
+ (BOOL) clusterAlreadyExistsForMapView:(MAMapView *)mapView andBlockCluster:(REVClusterBlock *)cluster
{
    for ( REVClusterPin *pin in mapView.annotations )
    {
        ...
        {
            MAMapPoint point1 =  MAMapPointForCoordinate([pin coordinate]);
            MAMapPoint point2 =  MAMapPointForCoordinate([[cluster getClusteredAnnotation] coordinate]);
            
            if( MAMapPointEqualToPoint(point1,point2) )
            {
                return YES;
            }
        }
...
{
    MAMapPoint mapRectPoints[5]={
        MAMapPointMake(mapRect.origin.x, mapRect.origin.y),
        MAMapPointMake(mapRect.origin.x+mapRect.size.width, mapRect.origin.y),
        MAMapPointMake(mapRect.origin.x+mapRect.size.width, mapRect.origin.y+mapRect.size.height),
        MAMapPointMake(mapRect.origin.x, mapRect.origin.y+mapRect.size.height),
        
        MAMapPointMake(mapRect.origin.x, mapRect.origin.y)
    };
    return [MAPolygon polygonWithPoints:mapRectPoints count:5];
}
