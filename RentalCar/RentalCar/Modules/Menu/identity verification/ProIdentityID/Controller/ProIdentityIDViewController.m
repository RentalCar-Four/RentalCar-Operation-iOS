...
//    debugLog(@"拍照");
    ...
    [UIAlertController showActionSheetInViewController:self withTitle:nil message:nil cancelButtonTitle:@"取消" destructiveButtonTitle:nil otherButtonTitles:@[@"拍照",@"相册选择"] popoverPresentationControllerBlock:^(UIPopoverPresentationController * _Nonnull popover) {
        ...
        if (buttonIndex==2) {//拍照
            [self takePhotoWithView:clickView];
        }else if (buttonIndex==3){
            [self openPhotoLibraryWithView:clickView];
        }
...
#pragma mark - 打开相册
- (void)openPhotoLibraryWithView:(UIImageView *)clickView{
    //本地相册
    if ([APPUtil isPhotoPermissionOn]) {
        ...
        pickerCl.isResizable = YES;
        pickerCl.clipSize = CGSizeMake(SCREEN_WIDTH, SCREEN_WIDTH/2*3);
...
//打开相机
- (void)takePhotoWithView:(UIImageView *)clickView{
...
//            debugLog(@"%@",_PhotoImgView);
            ...
            [_ClearBtn.superview bringSubviewToFront:_ClearBtn];
        }
            break;
        case 2:
        {
