...
- (void) shutterCamera
{
    ...
    [self.ImageOutPut connectionWithMediaType:AVMediaTypeVideo].videoMirrored = NO;
    if (!videoConnection) {
        NSLog(@"take photo failed!");
        return;
    }
    if (![APPUtil isCanUsePhotos]) {
        return;
    }
    ...
    [self.ImageOutPut captureStillImageAsynchronouslyFromConnection:videoConnection completionHandler:^(CMSampleBufferRef imageDataSampleBuffer, NSError *error) {
        ...
        self.imageView = [[UIImageView alloc]initWithFrame:CGRectMake(-(kScreenWidth/2-autoScaleW(280)/2), -(kScreenHeight/2-autoScaleH(400)/2), kScreenWidth, kScreenHeight)];
        self.imageView.image = _image;
        UIView *vvv = [[UIView alloc]initWithFrame:CGRectMake(kScreenWidth/2-autoScaleW(280)/2, kScreenHeight/2-autoScaleH(400)/2, autoScaleW(280), autoScaleH(400))];
        [vvv addSubview:self.imageView];
        [self.view addSubview:vvv];
        [self.view insertSubview:vvv belowSubview:_PhotoButton];
        UIImage *resultImg = [UIImage imageWithCGImage:[self snapshot:vvv].CGImage scale:2.0 orientation:UIImageOrientationLeft];
        [self.scnDriveDelegate onImg:resultImg];
