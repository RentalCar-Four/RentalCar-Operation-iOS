...
{
    UIImageView *backImgColor;
}
@property (weak, nonatomic) IBOutlet UILabel *titltLB;
...
//    [[APPUtil share] setButtonClickStyle:self.RechargeBtn normalColor:kBlueColor selectedColor:UIColorFromRGB(0x09C58A) cornerRadius:autoScaleW(5)];
    ...
    if (IS_IPhoneX) {
        backImgColor.top = -24;
        backImgColor.height = self.headView.height+24;
        self.headView.top = 24;
        self.CouponCell.top = self.headView.bottom;
        self.DepositCell.top = self.CouponCell.bottom;
//        self.whitebutton.top = self.whitebutton.
    }
...
//    [_loadingImgView setBackgroundColor:[UIColor redColor]];
    ...
    backImgColor = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, self.headView.width, self.headView.height)];
    backImgColor.image = image;
    backImgColor.contentMode = UIViewContentModeScaleToFill;
    backImgColor.autoresizingMask = UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleWidth;
    [self.headView insertSubview:backImgColor atIndex:0];
...
-(void)pushDepositView:(id)sender{
    ...
    }else if ([_qianModel.deposit floatValue]>0 &&[_qianModel.memberState intValue] ==3){
...
//        [CustomAlertView alertWithMessage:@"请先完成认证再来充值"];
        [APPUtil isAccountStatePass:self];
        
    }else if ([userStatus isEqualToString:@"4"] && [[UserInfo share].depositToPay floatValue]>0){
...
-(void)onSuccess:(id)data withType:(ActionType)type{
    switch (type) {
        case _REQUEST_MemberAccount_:
        {
            ...
            if ([_qianModel.deposit floatValue]>0) {
                if ([_qianModel.memberState intValue] ==3) {
                    [self.PayState setText:@"退款中"];
                } else {
